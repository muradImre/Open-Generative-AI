...
Open Generative AI is a free, open-source AI image, video, cinema, and lip sync studio that brings creative workflows to everyone. No content filters, no prompt rejections, no guardrails — just full creative freedom. It supports text-to-image, image-to-image, text-to-video, image-to-video, and audio-driven lip sync generation across models like Flux, Nano Banana, Midjourney, Kling, Sora, Veo, Seedream, Infinite Talk, LTX Lipsync, Wan 2.2, and more — all from a sleek, modern interface you can self-host and customize.
...
> **Local inference is currently available only in the desktop app.**
...
Installers are output to the `release/` folder. You can publish pre-built binaries through your repository’s Releases page.
...
The `packages/studio` library is shared across the application so model updates in `packages/studio/src/models.js` are applied consistently throughout the project.

## 🔌 API Integration

The app communicates with [MuAPI](https://muapi.ai) using a two-step pattern:
...
Cloud generation features currently integrate with [MuAPI](https://muapi.ai). Local inference options can be used without a cloud API key.
